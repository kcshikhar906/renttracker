...
export default function TransactionDetailModal({ isOpen, onClose, transaction, onDelete }) {
    ...
    const generatePDF = async () => {
        try {
            ...
            doc.setFontSize(14);
            doc.setFont(undefined, 'bold');
            doc.text("PROPERTY & RESIDENT DETAILS", 14, 60);

            const tableData = [
                ["Property Asset:", transaction.propertyName],
                ["Resident/Tenant:", transaction.tenant || "System Record"],
                ["Category:", transaction.type === "RENT" ? "Housing Rental" : (transaction.utilityType || "Utility Bill")],
                ["Total Amount:", `$${transaction.amount.toLocaleString(undefined, { minimumFractionDigits: 2 })}`],
                ["Settlement Status:", transaction.status],
                ["Filing Date:", safeFormat(transaction.date, "MMMM dd, yyyy")]
            ];

            if (transaction.type === "RENT") {
                tableData.push(["Rental Period:", `${safeFormat(transaction.periodStart, "MMM dd, yyyy")} - ${safeFormat(transaction.periodEnd, "MMM dd, yyyy")}`]);
                if (metrics) {
                    tableData.push(["Total Duration:", metrics.displayText]);
                }
            }
            ...
            doc.setFontSize(12);
            doc.setFont(undefined, 'bold');
            doc.text("ADMINISTRATIVE NOTES", 14, currentY);

            doc.setFontSize(9);
            doc.setFont(undefined, 'italic');
            doc.setTextColor(100, 116, 139);
            const splitNotes = doc.splitTextToSize(transaction.notes || "No additional audit notes provided for this settlement.", 182);
            doc.text(splitNotes, 14, currentY + 7);

            // 4. Digital Documentation Link
            if (transaction.fileUrl) {
                const linkY = currentY + (splitNotes.length * 5) + 15;
                doc.setFontSize(12);
                doc.setFont(undefined, 'bold');
                doc.setTextColor(30, 41, 59);
                doc.text("VERIFIED DOCUMENTATION", 14, linkY);
            ...
            doc.setFontSize(8);
            doc.setTextColor(148, 163, 184);
            doc.text("Validated accounting data generated by RentTracker Ledger.", 105, 285, { align: "center" });

            doc.save(`Statement_${transaction.propertyName.replace(/\s+/g, '_')}_${format(new Date(), "yyyyMMdd")}.pdf`);
        } catch (err) {
